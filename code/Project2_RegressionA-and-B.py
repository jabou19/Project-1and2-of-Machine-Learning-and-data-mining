...
for k, (outer_train_idx, outer_test_idx) in enumerate(outer_cv.split(x_normalized, yy)):
    ...
    rlr_val_errors = []
    rlr_val_errors = np.zeros(len(lambdas))
    inner_errors = []
    for inner_train_idx, inner_val_idx in inner_cv.split(X_train_outer):
        ...
        for i, lmbd in enumerate(lambdas):
            ...
            rlr_val_errors[i] += val_error  # accumulate per lambda


                # Select optimal lambda based on validation errors
        
    # Average errors across K2 folds
    rlr_val_errors = rlr_val_errors/K2
    ...
    # --- Inner CV for ANN ---
    ann_val_errors = []
    ann_val_errors = np.zeros(len(n_hidden_units_list))
    for inner_train_idx, inner_val_idx in inner_cv.split(X_train_outer):
        inner_errors = []
        ...
        for i, h in enumerate(n_hidden_units_list):
    ...
    #  a baseline model. For regression
...
results_df.to_csv('resultsRegression.csv', index=False)
...
plt.title(" Regression Error per Fold")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()

# --- Paired t-tests: Compare models pairwise (Setup I - Box 11.3.4) ---
alpha = 0.05
# Extract test errors across folds
ann_err = results_df['ANN error'].values
rlr_err = results_df['RLR error'].values
base_err = results_df['Baseline error'].values

# Compare ANN vs RLR
print("------------ANN vs RLR----------------")
z1 = ann_err - rlr_err
CI1 = st.t.interval(1 - alpha, len(z1) - 1, loc=np.mean(z1), scale=st.sem(z1))
p1 = 2 * st.t.cdf(-np.abs(np.mean(z1)) / st.sem(z1), df=len(z1) - 1)
print("θ̂ = θ_ANN - θ_RLR  Point Estimate:", np.mean(z1), "CI:", CI1, "p-value:", p1)

# Compare ANN vs Baseline
print("------------ANN vs Baseline----------------")
z2 = ann_err - base_err
CI2 = st.t.interval(1 - alpha, len(z2) - 1, loc=np.mean(z2), scale=st.sem(z2))
p2 = 2 * st.t.cdf(-np.abs(np.mean(z2)) / st.sem(z2), df=len(z2) - 1)
print("θ̂ = θ_ANN - θ_Baseline  Point Estimate:", np.mean(z2), "CI:", CI2, "p-value:", p2)

# Compare RLR vs Baseline
print("------------RLR vs Baseline----------------")
z3 = rlr_err - base_err
CI3 = st.t.interval(1 - alpha, len(z3) - 1, loc=np.mean(z3), scale=st.sem(z3))
p3 = 2 * st.t.cdf(-np.abs(np.mean(z3)) / st.sem(z3), df=len(z3) - 1)
print("θ̂ = θ_RLR - θ_Baseline  Point Estimate:", np.mean(z3), "CI:", CI3, "p-value:", p3)
# %%
