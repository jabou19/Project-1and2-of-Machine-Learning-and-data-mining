...
for k, (train_idx, test_idx) in enumerate(outer_cv.split(X, y)):
    ...
    for h in n_hidden_units_list:
        ...
        for inner_train_idx, inner_val_idx in inner_cv.split(X_train_outer):
            ...
            net, _, _ = train_neural_net(model=model, loss_fn=torch.nn.BCELoss(), X=X_in_train, y=y_in_train, n_replicates=n_replicates, max_iter=max_iter,tolerance=1e-6)
    ...
    net, _, learning_curve = train_neural_net(
        model=final_model, loss_fn=torch.nn.BCELoss(),
